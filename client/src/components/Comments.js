            ...
            <Box sx={{ flex: 1 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                <Typography variant="subtitle2" sx={{ color: '#ffb347', fontWeight: 600, cursor: 'pointer' }}
                  onClick={() => window.location.href = `/profile/${comment.user.id}`}
                  tabIndex={0}
                  onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') window.location.href = `/profile/${comment.user.id}`; }}
                    ...
                    <Paper key={reply.id} sx={{ p: 2, mb: 1, bgcolor: 'rgba(0, 0, 0, 0.3)', border: '1px solid rgba(255, 179, 71, 0.1)' }}>
                      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                        <Avatar
                          ...
                          onClick={() => window.location.href = `/profile/${reply.user.id}`}
                        >
                        ...
                        <Box sx={{ flex: 1 }}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                            <Typography variant="caption" sx={{ color: '#ffb347', fontWeight: 600, cursor: 'pointer' }}
                              onClick={() => window.location.href = `/profile/${reply.user.id}`}
                              tabIndex={0}
                              onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') window.location.href = `/profile/${reply.user.id}`; }}
