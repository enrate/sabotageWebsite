    ...
    if (Array.isArray(events)) {
        ...
        if (event.name === 'logger_player_killed') {
          ...
          if (isFriendlyFire && killerIdentity) {
            ...
            const [stats] = await db.UserStats.findOrCreate({
              ...
              defaults: { userId: killerUser ? killerUser.id : null, armaId: killerIdentity, kills: 0, deaths: 0, teamkills: 0 }
            });
            await stats.increment('teamkills');
            await stats.update({ lastUpdated: new Date() });
            // squad_stats только если есть killerUser
          ...
          if (victimIdentity) {
            ...
            const [stats] = await db.UserStats.findOrCreate({
              ...
              defaults: { userId: victimUser ? victimUser.id : null, armaId: victimIdentity, kills: 0, deaths: 0, teamkills: 0 }
            });
            await stats.increment('deaths');
            await stats.update({ lastUpdated: new Date() });
            // squad_stats только если есть victimUser
      ...
      if (killerIdentity) {
        ...
        const [stats, created] = await db.UserStats.findOrCreate({
          where: { armaId: killerIdentity },
          defaults: { userId: killerUser ? killerUser.id : null, armaId: killerIdentity, kills: 0, deaths: 0 }
        });
        ...
        // --- player_season_stats --- (ВСЕГДА, если есть killerIdentity и seasonId)
        if (seasonId) {
      ...
      if (victimIdentity) {
        ...
        const [stats, created] = await db.UserStats.findOrCreate({
          where: { armaId: victimIdentity },
          defaults: { userId: victimUser ? victimUser.id : null, armaId: victimIdentity, kills: 0, deaths: 0 }
        });
        await stats.increment('deaths');
        await stats.update({ lastUpdated: new Date() });
        // squad_stats только если есть victimUser
